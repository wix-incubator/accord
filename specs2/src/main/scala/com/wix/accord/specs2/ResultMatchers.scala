...
trait ResultMatchers[ Domain <: Constraints with Results { type Constraint >: Null <: AnyRef } ] {
  ...
  case class RuleViolationMatcher( value: Any = null, constraint: Domain#Constraint = null, description: String = null )
    ...
    def apply[ T <: Domain#Violation ]( left: Expectable[ T ] ) = left.value match {
  ...
  case class ResultMatcher( expectedViolations: Set[ ViolationMatcher ] ) extends Matcher[ Domain#Result ] {
    def apply[ T <: Domain#Result ]( left: Expectable[ T ] ) =
      left.value.fold(
        ifSuccess = result( test = false, "Validation was not successful", "Validation was successful", left ) )(
        ifFailure = { failure =>
          import scala.language.existentials
          val matched = failure.violations.map { v => ( v, expectedViolations.find( _ test v ) ) }
  ...
  /** A convenience method for matching failures. Enables syntax like:
    ...
    */
  def failWith( expectedViolations: ViolationMatcher* ): Matcher[ Domain#Result ] = ResultMatcher( expectedViolations.toSet )
  ...
  val fail = new Matcher[ Domain#Result ] {
    def apply[ T <: Domain#Result ]( left: Expectable[ T ] ) =
      result( test = left.value.isInstanceOf[ Domain#Failure ],
  ...
  val succeed = new Matcher[ Domain#Result ] {
    def apply[ T <: Domain#Result ]( left: Expectable[ T ] ) = {
      result( test = left.value.isSuccess,
        s"${left.description} is a success",
        s"${left.description} is not a success (violations: ${left.value.failure.map { _.violations }.getOrElse( Set.empty ).mkString( ", " )})",
        left )
    }
  }
}
